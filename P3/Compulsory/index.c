...
index_t* index_open(char* path) {
	...
	f=fopen(path, "r+");
  	...
  	while(ftell(f) != lastPos){
  	  	...
  	  	fread(&(record->key), sizeof(record->key), 1, f);
...
int index_save(index_t* index) {
	...
	for(i=0; i<index->numRecords; i++){
		...
		fwrite(&(record->key), sizeof(record->key), 1, f);
...
*/
int indexKeyExists(index_t *index, int key){
	int m, P, U;
	P=0;
	...
	while(P<=U){
		m=(P+U)/2;	
		if(index->records[m]->key == key){
			return m;
		}
		if(key< index->records[m]->key){
			U=m-1;
		}
		else{
			P=m+1;
...
int index_put(index_t *index, int key, long pos) {
	...
	if(i != -1){
		...
		for(i=index->numRecords-1; i>=0 && index->records[i]->key > key; i--){
